...
- (void)setupBlankSlate {
    //设置空白状态配置
    YCBlankSlateGeneralHandler *handler = [[YCBlankSlateGeneralHandler alloc] init];
    
    [handler setBackgroundColor:[UIColor clearColor] forState:YCDataLoadStateLoading | YCDataLoadStateEmpty | YCDataLoadStateFailed];
    ...
    //加载状态配置
    [handler setTitle:@"加载中..." forState:YCDataLoadStateLoading];
    [handler setImage:[UIImage imageNamed:@"loading"] forState:YCDataLoadStateLoading];
    ...
    }) forState:YCDataLoadStateLoading];
    [handler setAnimate:YES forState:YCDataLoadStateLoading];
    
    //无数据状态配置
    [handler setTitle:@"数据为空" forState:YCDataLoadStateEmpty];
    [handler setDescription:@"糟糕！这里什么都没有~" forState:YCDataLoadStateEmpty];
    [handler setImage:[UIImage imageNamed:@"empty"] forState:YCDataLoadStateEmpty];
    
    //加载错误状态配置
    [handler setImage:[UIImage imageNamed:@"error"] forState:YCDataLoadStateFailed];
    [handler setAttributedDescription:({
        ...
        string;
    }) forState:YCDataLoadStateFailed];
    [handler setButtonTitle:@"重新载入" controlState:UIControlStateNormal forState:YCDataLoadStateFailed];
    ...
    [handler setButtonBackgroundImage:bgImage controlState:UIControlStateNormal forState:YCDataLoadStateFailed];
    [handler setButtonBackgroundImage:bgImage controlState:UIControlStateHighlighted forState:YCDataLoadStateFailed];
    [handler setSpaceHeight:20 forState:YCDataLoadStateFailed];
    ...
    self.tableView.yc_handler = handler;
}
/**********************************/

- (void)loadData {
    //显示加载状态页
    if (self.mode == YCDisplayModeLoading) {
        self.tableView.yc_handler.state = YCDataLoadStateLoading;
        return;
    }
    //显示空数据页
    if (self.mode == YCDisplayModeEmpty) {
        self.tableView.yc_handler.state = YCDataLoadStateLoading;
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            self.tableView.yc_handler.state = YCDataLoadStateEmpty;
        });
        return;
    }
    //显示加载错误页
    if (self.mode == YCDisplayModeFail) {
        self.tableView.yc_handler.state = YCDataLoadStateLoading;
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            self.tableView.yc_handler.state = YCDataLoadStateFailed;
        });
        return;
    }
    //显示正常加载页
    if (self.mode == YCDisplayModeSuccess) {
        self.tableView.yc_handler.state = YCDataLoadStateLoading;
        [self pullToRefresh];
    }
}
...
- (void)pullToRefresh {
    ...
    [self.viewModel queryListWithPage:self.page success:^{
        ...
        if (self.viewModel.list.count == 0) {
            ...
            //有数据的情况
            //置为闲置状态
            self.tableView.yc_handler.state = YCDataLoadStateIdle;
            //设置footer显示状态
